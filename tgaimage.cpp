...
bool TGAImage::set_with_blend(int x, int y, const TGAColor& c) {
    if (!data || x < 0 || y < 0 || x >= width || y >= height) {
        return false;
    }

    if (bytespp == 4 && c.bytespp == 4 && c.bgra[3] < 255) {
        TGAColor bg = get(x, y);
        TGAColor blended = TGAColor::blend(bg, c);
        memcpy(data + (x + y * width) * bytespp, blended.bgra, bytespp);
        return true;
    }
    else {
        return set(x, y, c);
